    ...
    try:
        ...
        if account_id:
            try:
                ...
                # Fetch all instances; we filter by engine client-side for reliability
                response = rds.describe_db_instances()
                databases = []
                for db in response.get('DBInstances', []):
                    ...
                    if not host:
                        continue
                    # Normalize engine from RDS Engine field (from IAM-permitted describe)
                    ...
                    # Filter by selected tab's engine - only show matching instances
