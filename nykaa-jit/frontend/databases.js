...
async function renderDbStepContent() {
    ...
    if (provider === 'aws') {
        ...
        if (step === 1) {
            ...
            content.innerHTML = `<div class="db-step-loading"><i class="fas fa-spinner fa-spin"></i> Fetching ${selectedEngine?.label || wantEngine} instances…</div>`;
            const result = await fetchDatabasesForAccount(accountId, wantEngine);
            const instances = filterDatabasesByEngine(result.databases || [], wantEngine);
            if (result.error) {
            ...
            content.innerHTML = `
                ${result.error ? `<div class="db-step-error-bar"><span class="db-error-reopen" onclick="reopenDbErrorPopup()" title="View error details"><i class="fas fa-exclamation-triangle"></i> Unable to list RDS — Click to see instructions</span> <button class="btn-secondary btn-sm" onclick="retryDbFetch()" style="margin-left:8px">Retry</button></div>` : ''}
                <div class="db-step-field">
            ...
            const accountId = document.getElementById('dbStepAccount')?.value || dbRequestDraft?.account_id;
            const wantEngine = (selectedEngine?.engine || '').toLowerCase();
            content.innerHTML = `<div class="db-step-loading"><i class="fas fa-spinner fa-spin"></i> Fetching ${selectedEngine?.label || wantEngine} databases…</div>`;
            const result = await fetchDatabasesForAccount(accountId, wantEngine);
            ...
            content.innerHTML = `
                ${result.error ? `<div class="db-step-error-bar"><span class="db-error-reopen" onclick="reopenDbErrorPopup()" title="View error details"><i class="fas fa-exclamation-triangle"></i> Unable to list databases — Click to see instructions</span> <button class="btn-secondary btn-sm" onclick="retryDbFetch()" style="margin-left:8px">Retry</button></div>` : ''}
                <div class="db-step-field">
...
function connectToDatabase(host, port, engine, requestId, dbName) {
    ...
    if (typeof showPage === 'function') {
        ...
        alert('Go to DB Terminal page from the sidebar to connect.');
    }
}
